...
| Menu configurable por perfil ("vistas" del sistema) + Administrador de Menu | 000194 | CONSTRUIDO | ADR-0030; menu del workspace data-driven por vista/perfil (MenuView+MenuNode), asignacion usuario->vista, editor /configuracion-menu (arbol, propiedades, iconos, vista previa, drag-and-drop). Reusa el stub 000194 |
| Administracion de usuarios del tenant | 000073 | CONSTRUIDO | ADR-0031; /admin-usuarios (invitar/rol/estado/reset clave/asignar vista de menu) reusando ITenantUserService |
| Roles y permisos dinamicos + enforcement | 000198 | CONSTRUIDO | ADR-0032/0033 (apoyo en Visal); Rol+RolPermiso matriz Modulo x Ver/Crear/Editar/Eliminar (catalogo derivado del menu), /roles-permisos, TenantUser.RolId; enforcement real (policy provider dinamico + menu filtrado por Ver + botones gateados), regla opt-in Owner/Admin/sin-rol=Unrestricted. Es el "paso 2" del Module Registry aplicado |
