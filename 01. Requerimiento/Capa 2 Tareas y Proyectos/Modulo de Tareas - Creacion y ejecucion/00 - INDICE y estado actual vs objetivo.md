...
| Runtime de flujos (nodo->cargo, cargo->usuarios, reclamar/atender/aprobar en la tarea, motor Advance/Reject) | **Construido y maduro** | `WorkflowEngine.cs`, `WorkflowInboxService.cs`, `WorkflowNodePolicy.cs`, `OrgAssigneeTree.cs`, `INodeAssigneeResolver.cs`; **embebido en `TaskDetailModal.razor` (seccion Flujo)**. Los pasos pendientes se descubren en el **TABLERO** (alcance "mis pendientes"); **NO hay bandeja/pagina "Mis pasos" aparte** (ADR-0038, retira `/mis-pasos` + item 000637) |
